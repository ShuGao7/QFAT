...
!  ----------------------------------------------------------------------------
      ...
      SUBROUTINE GRD2DEM_Call(infile)
      ...
      CHARACTER*400                      :: gridfile
      CHARACTER*400                       :: demfile
      CHARACTER*400                       :: infile
      CHARACTER*400                       :: ofile
      CHARACTER*400                      :: tfile
      CHARACTER*24                        :: AGRID
      CHARACTER*400                       :: dumC
      ...
      REAL(8),ALLOCATABLE                 :: WSE(:),TIME (:)

      REAL(4), ALLOCATABLE                :: Raster_Z(:,:)

      LOGICAL                             :: found,maxfile,timefile
      ...
      WRITE(*,*)""
      WRITE(*,'(A)',ADVANCE='YES')"-------------------------------------------"
      WRITE(*,'(A)',ADVANCE='YES')"               MESH2TIME                   "
      WRITE(*,'(A)',ADVANCE='YES')"       MATTHEW BILSKIE, SHU GAO            "
      WRITE(*,'(A)',ADVANCE='YES')"                LSU CCR                    "
      WRITE(*,'(A)',ADVANCE='YES')"            COPYRIGHT 2017                "
      WRITE(*,'(A)',ADVANCE='YES')"-------------------------------------------"
      WRITE(*,*)""
      ...
      loopA: DO i=1,LEN_TRIM(dumC)
                DO j = 1,LEN_TRIM(dumC)
             
          IF(dumC(i:i).EQ.",")THEN
              gridfile=(dumC(1:i-1))
              IF (dumC(j:j).EQ.";")THEN
                    ofile = (dumC(i+1:j-1))
                    tfile = TRIM(dumC(j+1:LEN_TRIM(dumC)))
              ENDIF  
      ...
      IF (timefile) THEN
          INQUIRE(FILE = TRIM(tfile),EXIST=found) ! CHECKS TO SEE IF INPUT FILE EXISTS
          IF (found) THEN
              WRITE(*,1010)tfile
              OPEN(6633,FILE=tfile,STATUS='OLD',ACTION='READ')
          ELSE
              WRITE(*,1011)ofile
              WRITE(*,'(A)',ADVANCE='YES')"PRESS ANY KEY TO EXIT"
              READ(*,*)
      ...
      IF (tfile.NE."") THEN
          READ(6633,*)
          READ(6633,*)tempT,dumT
          IF (dumT.ne.NP) THEN
              WRITE(*,*)'GEOMETRY IN ',TRIM(tfile),' DOES NOT MATCH GRID FILE.'
              WRITE(*,*)
              STOP
          ENDIF
          READ(6633,*)
      ...
      DO i = 1, NP
          ...
          IF(timefile) READ(6633,*)dumT,TIME(i)
          ...
          IF (ID_Flag.EQ.1) THEN
            ...
            ELSE
                z(i) = -99999.0D0
            ENDIF
          ELSEIF (timefile) THEN
            IF((maxvalue==180).AND.(minvalue==0)) THEN
                IF((WSE(i).NE.-99999).AND.(TIME(i).NE.0).AND.(z(i).LT.0)) THEN
                    z(i) = TIME(i)
                ELSE 
                    Z(i) = -99999.0D0
                ENDIF
            ELSE
                IF((WSE(i).NE.-99999).AND.(TIME(i).NE.0).AND.(z(i).LT.0).AND.(TIME(i).LE.maxvalue).AND. (TIME(i).GE.minvalue))THEN
                    z(i) = TIME(i)
                ELSE 
                    Z(i) = -99999.0D0
                ENDIF
...
1010  format(' FILE ',a400,/,'WAS FOUND!   OPENING FILE',/)      
1011  format(' FILE ',a400,/,'WAS NOT FOUND!  PROGRAM TERMINATING',/)
